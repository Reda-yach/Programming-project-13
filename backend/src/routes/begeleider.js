...
//   - 'meer_info'    -> stage.status 'aanpassing_vereist'  (motivatie verplicht)
//   - 'afgewezen'    -> stage.status 'afgewezen'           (motivatie verplicht)

const BESLISSING_NAAR_STATUS = {
  goedgekeurd: 'goedgekeurd',
  meer_info: 'aanpassing_vereist',
  afgewezen: 'afgewezen',
};
...
router.put('/:id/beslissing', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!['ingediend', 'in_behandeling', 'aanpassing_vereist'].includes(huidigeStatus)) {
      await conn.rollback();
      return res.status(400).json({
